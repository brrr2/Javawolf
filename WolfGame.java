...
public class WolfGame {
    ...
    private boolean loadPConfig(String filepath) {
        ...
        // Load file.
        wolfBot.logEvent("Loading \"" + filepath + ".cfg\"....", Javawolf.LOG_PLAYERCONFIG, false);
        try {
            cfg = new BufferedReader(new FileReader(filepath + ".cfg"));
        } catch(FileNotFoundException e) {
            wolfBot.logEvent("Could not find file \"" + filepath + "\"!", Javawolf.LOG_PLAYERCONFIG, true);
            return false;
        }
        ...
        try {
            while (cfg.ready()) {
                ...
                if (charLoc != -1) {
                    ...
                    if (!isSettingRoles) {
                        if (variable.equals("playerconfig")) {
    ...
    public void parseCommand(String cmd, String[] args, String nick, String user, String host) {
        if(cmd.equals("join")) {
            ...
            // Lists all the roles
            roles(nick, user, host);
        } else if (cmd.equals("simple")) {
            // Toggles simple status
            simple(nick, user, host);
        } else if (cmd.equals("ping")) {
            // Echoes each nick in the channel
            ping(nick, user, host);
        } else if (cmd.equals("help")) {
            // Outputs a list of commands or usage of a command
            if (args != null && args.length > 1) {
                help(args[1], nick, user, host);
            } else {
                help(null, nick, user, host);
            }
        } else if (cmd.equals("rules")) {
            rules(nick, user, host);
    ...
    private void setvar(String var, String val, String nick, String user, String host) {
        ...
        if(var.equals("allowcursedseer")) {
            ...
            try {
                ...
                wolfBot.logEvent("Could not parse \"" + val + "\"!", Javawolf.LOG_ADMIN, true);
            }
        }
    }
    ...
    private void waitstart(String nick, String user, String host) {
        // Is game running?
        if(isRunning) {
            msg(nick, "The game is already running.");
            return;
        }
        // Is the user playing?
    ...
    private void myrole(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the MYROLE command", Javawolf.LOG_GAME, false);
        
        WolfPlayer p = getPlayer(nick, user, host);
        // Is game running?
        if (!isRunning) {
            msg(nick, "The game is not running.");
    ...
    private void stats(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the STATS command", Javawolf.LOG_GAME, false);
    ...
    private void votes(String nick, String user, String host){
        // Logs the command
        wolfBot.logEvent(nick + " issued the VOTES command", Javawolf.LOG_GAME, false);
        
        WolfPlayer p = getPlayer(nick, user, host);
        // Is the game even running?
        if(!isRunning) {
            msg(nick, "No game is currently running.");
        // Is the player even playing?
        } else if(p == null) {
            msg(nick, "You aren't even playing.");
        // Is the player alive?
        } else if(!isAlive(p)) {
            msg(nick, "You have died already and thus cannot use the votes command.");
        // Is it night
        } else if(isNight){
            msg(nick, "It is currently night.");
    ...
    private void curse(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the CURSE command", Javawolf.LOG_GAME, false);
        ...
        // Has a target been selected
            ...
            msg(nick, "You must select a target.");
    ...
    private void raise(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the RAISE command", Javawolf.LOG_GAME, false);
        ...
        // Is the player a medium?
            ...
            msg(nick, "Only mediums can raise other players.");
        // Is it day?
        } else if (isNight) {
            msg(nick, "You may only raise players during the day.");
    ...
    private void guard(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the GUARD command", Javawolf.LOG_GAME, false);
        ...
        // Is the player a guardian angel?
            ...
            msg(nick, "Only guardian angels can guard other players.");
        // Is it night?
        } else if(!isNight) {
            msg(nick, "You may only guard during the night.");
    ...
    private void observe(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the OBSERVE command", Javawolf.LOG_GAME, false);
    ...
    private void id(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the ID command", Javawolf.LOG_GAME, false);
    ...
    private void shoot(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the SHOOT command", Javawolf.LOG_GAME, false);
        ...
        // He can now fire.
            ...
            if((p.countWolfRoles() > 0) && (target.isWolf() || target.isWerecrow())) {
    ...
    private void visit(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the VISIT command", Javawolf.LOG_GAME, false);
    ...
    private void see(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the SEE command", Javawolf.LOG_GAME, false);
    ...
    private void tavernKick(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the TAVERNKICK command", Javawolf.LOG_GAME, false);
    ...
    private void tavernInvite(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the TAVERNINVITE command", Javawolf.LOG_GAME, false);
    ...
    private void kill(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the KILL command", Javawolf.LOG_GAME, false);
    ...
    private void lynch(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the LYNCH / RETRACT command", Javawolf.LOG_GAME, false);
    ...
    private void join(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the JOIN command", Javawolf.LOG_GAME, false);
        ...
        // Has player already joined?
            ...
            msg(nick, "You have already joined the game.");
    ...
    private void leave(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the LEAVE command", Javawolf.LOG_GAME, false);
    ...
    private void startgame(String nick, String user, String host, boolean forced) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the START command", Javawolf.LOG_GAME, false);
        ...
        if (isRunning) {
            msg(nick, "The game has already begun.");
        // Is the starter even in the game?
        } else if (p == null) {
            msg(nick, "You're currently not joined.");
        // Waited long enough?
        ...
        // 4+ people?
            ...
            wolfBot.logEvent("Getting config....", Javawolf.LOG_GAME, false);
            // Gets the configuration
            WolfConfig wc = getConfig(getNumPlayers());
            if(wc == null) {
                chanmsg("Configuration error: " + getNumPlayers() + " players is not supported.");
                wolfBot.logEvent(getNumPlayers() + " is an unsupported player count.", Javawolf.LOG_GAME, true);
                return;
            }
            ...
            wolfBot.logEvent("Welcoming players....", Javawolf.LOG_GAME, false);
            String namelist = "";
            WolfPlayer temp;
            for (int m = 0; m < getNumPlayers(); m++) {
                temp = getPlayer(m);
            ...
            // Assign the roles
            wolfBot.logEvent("Assigning roles....", Javawolf.LOG_GAME, false);
    ...
    private void help(String command, String nick, String user, String host){
        // Logs the command
        wolfBot.logEvent(nick + " issued the HELP command", Javawolf.LOG_GAME, false);
        
        if (command == null) {
            msg(nick, "Commands: help, simple, back, quit, votes, stats, away, ping, start, retract, goat, shoot, lynch, coin, wait, join, roles, rules, admins");
        } else {
            if (command.equalsIgnoreCase("help")) {
                msg(nick, "!help: Gets help");
            } else if (command.equalsIgnoreCase("simple")) {
                msg(nick, "!simple: If you want the bot to NOTICE you for every interaction");
            } else if (command.equalsIgnoreCase("back")) {
                msg(nick, "!back: Unmarks away status");
            } else if (command.equalsIgnoreCase("away")) {
                msg(nick, "!away: Use this to activate your away status (so you aren't pinged).");
            } else if (command.equalsIgnoreCase("quit")) {
                msg(nick, "!quit: Quits the game.");
            } else if (command.equalsIgnoreCase("leave")) {
                msg(nick, "!leave: Quits the game.");
            } else if (command.equalsIgnoreCase("votes")) {
                msg(nick, "!votes: Displays the voting statistics.");
            } else if (command.equalsIgnoreCase("stats")) {
                msg(nick, "!stats: Display the player statistics");
            } else if (command.equalsIgnoreCase("ping")) {
                msg(nick, "!ping: Pings the channel to get people's attention. Rate-Limited.");
            } else if (command.equalsIgnoreCase("start")) {
                msg(nick, "!start: Starts a game of Werewolf");
            } else if (command.equalsIgnoreCase("retract")) {
                msg(nick, "!retract: Takes back your vote during the day (for whom to lynch)");
            } else if (command.equalsIgnoreCase("goat")) {
                msg(nick, "!goat: Use a goat to interact with anyone in the channel during the day");
            } else if (command.equalsIgnoreCase("shoot")) {
                msg(nick, "!shoot: Use this to fire off a bullet at someone in the day if you have bullets");
            } else if (command.equalsIgnoreCase("lynch")) {
                msg(nick, "!lynch: Use this to vote for a candidate to be lynched");
            } else if (command.equalsIgnoreCase("vote")) {
                msg(nick, "!vote: Use this to vote for a candidate to be lynched");
            } else if (command.equalsIgnoreCase("coin")) {
                msg(nick, "!coin: It's a bad idea to base any decisions on this command.");
            } else if (command.equalsIgnoreCase("wait")) {
                msg(nick, "!wait: Increase the wait time (before !start can be used)");
            } else if (command.equalsIgnoreCase("rules")) {
                msg(nick, "!rules: Displays the rules");
            } else if (command.equalsIgnoreCase("roles")) {
                msg(nick, "!roles: Display which roles are enabled and when");
            } else if (command.equalsIgnoreCase("join")) {
                msg(nick, "!join: Either starts a new game of Werewolf or joins an existing game that has not started yet.");
            } else if (command.equalsIgnoreCase("admins")) {
                msg(nick, "!admins: Pings the admins that are available.");
            } else {
                msg(nick, "Command not found.");
            }
        }
    }
    
    private void ping(String nick, String user, String host){
        // Logs the command
        wolfBot.logEvent(nick + " issued the PING command", Javawolf.LOG_GAME, false);
        ...
        if (isRunning) {
            ...
            for (int ctr = 0; ctr < users.length; ctr++) {
                outStr += " " + users[ctr].getNick();
            }
            chanmsg(outStr);
        }
    }
    
    private void roles(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the ROLES command", Javawolf.LOG_GAME, false);
    }
    ...
    private void checkForIdles() {
        // Sanity check.
        if (!isRunning) {
            wolfBot.logEvent("Idler check run when game is off.", Javawolf.LOG_GAME, true);
        }
    ...
    private boolean checkEndNight() {
        ...
        for (int m = 0; m < getNumPlayers(); m++) {
            ...
            if (isAlive(temp)) {
                if (temp.isSeer() && temp.seen == null) {
                    return false; // Seer didn't see
                }
                if (temp.isHarlot() && temp.visited == null) {
                    return false; // Harlot didn't visit
                }
                if (temp.isWolf() && temp.voted == null) {
                    return false; // Wolf didn't kill
                }
                if (temp.isAngel() && temp.guarded == null) {
                    return false; // Angel hasn't guarded
                }
                if (temp.isWerecrow() && temp.voted == null && temp.observed == null) {
                    return false; // Crow hasn't killed or observed
                }
                if (temp.isSorcerer() && temp.cursed == null) {
                    return false; // Sorcerer hasn't cursed
                }
            }
        }
    ...
    /**
     * Checks to see if the villagers or wolves win. Returns true if the game
    ...
    /**
     * Counts the number of wolves left. Used for turning traitors into wolves.
     *
     * @return
     */
    ...
    private void startNight() {
        wolfBot.logEvent("Starting the night.", Javawolf.LOG_GAME, false);
    ...
    private void endNight() {
        wolfBot.logEvent("Ending the night.", Javawolf.LOG_GAME, false);
        timer.cancel();
    ...
    /**
     * Tallies the votes. First value is the number of votes, second value is
     * the person most voted for. The second value is -1 when a tie occurs.
     *
     * @return
     */
    ...
    private void fquit(String nick, String user, String host) {
        ...
        wolfBot.logEvent(nick + "!" + user + "@" + host + " has shut down this bot.", Javawolf.LOG_ADMIN, false);
        System.exit(0);
    }
    ...
    private void fjoin(String who, String nick) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the FJOIN command", Javawolf.LOG_ADMIN, false);
            ...
            for (int ctr = 0; ctr < users.length; ctr++) {
                if (users[ctr].getNick().equalsIgnoreCase(who)) {
                    ...
                    // Force join the user to the game.
                        ...
                        wolfBot.logEvent("Force joined (" + nick + "," + username + "," + hostmask + ")", Javawolf.LOG_ADMIN, false);
                        return;
                    }
                }
            }
    ...
    private void fleave(String who, String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the FLEAVE command on ", Javawolf.LOG_ADMIN, false);
        // get who this is
        ...
        if (isRunning) {
            ...
            if (rmvPlayer(p.getNick(), p.getUser(), p.getHost())) {
                ...
                wolfBot.logEvent(killedNick + " was ejected from the game.", Javawolf.LOG_ADMIN, false);
            } else {
    ...
    private void fendgame(String nick, String user, String host) {
        // Logs the command
        wolfBot.logEvent(nick + " issued the FENDGAME command", Javawolf.LOG_ADMIN, false);
        // notify channel
    ...
    private void playerDeath(WolfPlayer p) {
        ...
        // sanity check
        if (!isRunning) {
            wolfBot.logEvent("<Playerdeath> called while game is not running!", Javawolf.LOG_GAME, true);
            return;
        }
        // If in wolf channel, kick him out.
        ...
        if (villagers.contains(p)) {
            villagers.remove(p);
        }
        if (wolfTeam.contains(p)) {
            wolfTeam.remove(p);
        }
    ...
    public void resetIdle(String nick, String user, String host) {
        ...
        if (p != null && p.addAction(System.currentTimeMillis())) {
            ...
            wolfBot.logEvent("Kicking " + nick + " for flooding.", Javawolf.LOG_CONSOLE, false);
    ...
    private void ignoreHost(String who, String nick, String user, String host) {
        wolfBot.logEvent("\"" + who + "\" was added to ignore list by " + nick + ".", Javawolf.LOG_ADMIN, false);
    ...
    private void unignoreHost(String who, String nick, String user, String host) {
        wolfBot.logEvent("\"" + who + "\" was removed from the ignore list by " + nick + ".", Javawolf.LOG_ADMIN, false);
        if(!wolfBot.ignoredHosts.remove(who)) {
            // Couldn't remove target.
            wolfBot.logEvent("Could not find hostmask \"" + who + "\" in ignore list.", Javawolf.LOG_ADMIN, true);
    ...
    private WolfConfig getConfig(int num) {
        for (int m = 0; m < nCfgs; m++) {
            if (configs[m] == null) {
                // some bug
                wolfBot.logEvent("Config #" + m + " is null!", Javawolf.LOG_PLAYERCONFIG, true);
                return null;
            }
